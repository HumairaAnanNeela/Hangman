...
int main()
{
    ...
    system("PAUSE");// input er jnno wait kore
    while(1)
    {
        if(level==0)
        {
...
void start_game()
{
    ...
    if (level ==0)
    {
        ...
        for (int i = 0; i < sizeof(guessed_letters); i++) //2nd time khelte chaile guess letter ta jate 0 kore dey..
                                                         //   ager guess letter ta jate na thake
        {
            guessed_letters[i] = 0;// ini
        }
        attempts = 10;
    }
    if (level==1)
...
void get_input()
{
    ...
    int i;
    int letter_hit = 0; // used to tell the letter is in the word
